...
def load_model_for_compute():
    CHECKPOINTS = "/ml/home/mcldd/ner-rel/diabete-kno-dataset-code/mrc-for-flat-nested-ner-master/lightning_logs/version_17/checkpoints/epoch=7.ckpt"
    HPARAMS = "/ml/home/mcldd/ner-rel/diabete-kno-dataset-code/mrc-for-flat-nested-ner-master/lightning_logs/version_17/hparams.yaml"

    model = trainer.BertLabeling.load_from_checkpoint(
...
def load_dataset_for_compute():
    """test dataset"""
    import os
    from datasets.collate_functions import collate_to_max_length
    from torch.utils.data import DataLoader
    bert_path = "/ml/home/mcldd/ner-rel/diabete-kno-dataset-code/mrc-for-flat-nested-ner-master/models/chinese_roberta_wwm_large_ext_pytorch"
    json_path = "/ml/home/mcldd/ner-rel/diabete-kno-dataset-code/mrc-for-flat-nested-ner-master/entity_type_data/mrc-ner.test"
...
def write_json_to_file(origin_path, des_path, data):
    with open(origin_path + des_path, "w", encoding="utf8") as fp:
...
def compute_result_by_query():
    model = load_model_for_compute()
    data, tokenizer = load_dataset_for_compute()
    entity_total = {}
    with tqdm(total=len(data)) as pbar:
        try:
            for batch_idx, batch in enumerate(data):
                sample = data.dataset.all_data[batch_idx]
                entity_label = sample["entity_label"]
                if entity_label not in entity_total.keys():
                    entity_total[entity_label] = {"tp": 0, "fp": 0, "fn": 0}
                out_puts = model.validation_step(batch, batch_idx)
                tp, fp, fn = out_puts["span_f1_stats"]
                entity_total[entity_label]["tp"] = entity_total[entity_label]["tp"] + tp.item()
                entity_total[entity_label]["fp"] = entity_total[entity_label]["fp"] + fp.item()
                entity_total[entity_label]["fn"] = entity_total[entity_label]["fn"] + fn.item()
                pbar.update(1)
        except Exception as e:
            print("ERROR,PLEASE CHECK CODE！{}".format(e))
    entity_total_result = {}
    tp_total = 0
    fp_total = 0
    fn_total = 0
    for k, v in entity_total.items():
        tp = v["tp"]
        fp = v["fp"]
        fn = v["fn"]
        tp_total += tp
        ...
        entity_total_result[k] = {"precision": "{:.2f}%".format(P), "recall": "{:.2f}%".format(R), "f1": "{:.2f}%".format(F1)}
    P_Tol = tp_total * 100.0 / (tp_total + fp_total)
    R_Tol = tp_total * 100.0 / (tp_total + fn_total)
    F1_Tol = 0 if (P_Tol + R_Tol) == 0 else (2 * P_Tol * R_Tol) / (P_Tol + R_Tol)
    write_json_to_file("", "query_entity_result.txt", entity_total_result)
    print("total precision:{:.2f},total recall:{:.2f},total f1:{:.2f}".format(P_Tol, R_Tol, F1_Tol))


if __name__ == '__main__':
    # run_dataset()
    compute_result_by_query()
